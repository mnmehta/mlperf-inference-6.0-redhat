...
class Instance(mp.Process):
    ...
    def process_queries(self):
        try:
            qitem_list = self.input_queue.get(timeout=30)  # Wait up to 30 seconds
        except queue.Empty:
            return True  # Keep worker alive, just try again

        if qitem_list is None:
            return False  # Explicit None means shutdown
...
class vllmSUT:
    def __init__(
        ...
        self.query_queues = [mp.JoinableQueue() for _ in range(self.num_workers)]
    ...
    def start(self):
        ...
        for j in range(self.num_workers):
            worker = Instance(
                ...
                input_queue=self.query_queues[j],
    ...
    def issue_queries(self, query_samples):
        ...
        # Split into chunks and distribute round-robin to workers
        chunks = []
        for i in range(0, len(query_list), chunk_size):
            chunks.append(query_list[i : i + chunk_size])
        
        for i, chunk in enumerate(chunks):
            worker_idx = i % self.num_workers
            self.query_queues[worker_idx].put(chunk)
